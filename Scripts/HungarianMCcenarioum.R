...
###### DADOS
dados=read.table("Hungariandata.txt",na.strings = -9,header=T,dec=".",sep = "\t")
attach(dados)
...
# 3 - Y~sex+age+IPPA
# 4 - Y~sex+age+htn+PAM+RC+IPPARC+ALPHA2
# 5 - Y~sex+age+htn+PAM+RC+IPPARC
...
modNB3=naiveBayes(Y~sex+age+IPPA,data = dadostrei) #### MELHOR
YestNB3trei=predict(modNB3,newdata = dadostrei);YestNB3test=predict(modNB3,newdata = dadostest)

modNB4=naiveBayes(Y~sex+age+htn+PAM+RC+IPPARC+ALPHA2,data = dadostrei)
YestNB4trei=predict(modNB4,newdata = dadostrei);YestNB4test=predict(modNB4,newdata = dadostest)

modNB5=naiveBayes(Y~sex+age+htn+PAM+RC+IPPARC,data = dadostrei)
...
DadostestRF3=na.omit(data.frame(dadostest$Y,dadostest$sex,dadostest$age,dadostest$IPPA))
RF3=randomForest(Y~sex+age+IPPA,data = dadostrei,xtest=DadostestRF3[,-1],ytest=DadostestRF3$dadostest.Y,na.action = na.omit)

DadostestRF4=na.omit(data.frame(dadostest$Y,dadostest$sex,dadostest$age,dadostest$htn,dadostest$PAM,dadostest$RC,dadostest$IPPARC,dadostest$ALPHA2))
RF4=randomForest(Y~sex+age+htn+PAM+RC+IPPARC+ALPHA2,data = dadostrei,xtest=DadostestRF4[,-1],ytest=DadostestRF4$dadostest.Y,na.action = na.omit)

DadostestRF5=na.omit(data.frame(dadostest$Y,dadostest$sex,dadostest$age,dadostest$htn,dadostest$PAM,dadostest$RC,dadostest$IPPARC))
RF5=randomForest(Y~sex+age+htn+PAM+RC+IPPARC,data = dadostrei,xtest=DadostestRF5[,-1],ytest=DadostestRF5$dadostest.Y,na.action = na.omit)
...
modSVM3=svm(Y~sex+age+IPPA,scale=F,data=dadostrei,kernel="poly",cost=100,epsilon=1.0e-12)
YestSVM3test=predict(modSVM3,dadostest);YestSVM3trei=modSVM3$fitted

modSVM4=svm(Y~sex+age+htn+PAM+RC+IPPARC+ALPHA2,data=dadostrei,scale=F,kernel="poly",cost=100,epsilon=1.0e-12)
YestSVM4test=predict(modSVM4,dadostest);YestSVM4trei=modSVM4$fitted

modSVM5=svm(Y~sex+age+htn+PAM+RC+IPPARC,data=dadostrei,scale=F,kernel="poly",cost=100,epsilon=1.0e-12)
...
modLR3=glm(Y~sex+age+IPPA,data=dadostrei,family = binomial(link = "logit"))
...
modLR4=glm(Y~sex+age+htn+PAM+RC+IPPARC+ALPHA2,data=dadostrei,family = binomial(link = "logit"))
...
modLR5=glm(Y~sex+age+htn+PAM+RC+IPPARC,data=dadostrei,family = binomial(link = "logit"))
